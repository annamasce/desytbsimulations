...
void PrimaryGeneratorAction::SetDefaultKinematic()
{
  ...
  //G4double angle = (G4UniformRand()-0.5)*PI/6;
  //G4double angle2 = (G4UniformRand()-0.5)*PI/6;
  //particleGun->SetParticleMomentumDirection(G4ThreeVector(cos(angle),sin(angle),sin(angle2)));
  ...
  //particleGun->SetParticlePosition(G4ThreeVector(position,0.*mm,-49.01*mm));//,0.*mm));
}
...
void PrimaryGeneratorAction::GeneratePrimaries(G4Event* anEvent)
{
    ...
    G4double pos = -0.5*(Detector->GetWorldSizeX());
    G4double z=0.*mm;
    G4double y=0.*mm;
    //randomize beam, if requested.
    particleGun->SetParticlePosition(G4ThreeVector(pos,y,z));
    G4double angle = (G4UniformRand()-0.5)*0.00001; // angle X
    G4double angle2 = (G4UniformRand()-0.5)*0.00001; // angle Y
    particleGun->SetParticleMomentumDirection(G4ThreeVector(cos(angle),sin(angle),sin(angle2)));
