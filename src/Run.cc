...
void Run::SetPrimary(G4ParticleDefinition* particle, G4double energy, G4ThreeVector position)
{ 
  fParticle = particle;
  fEkin = energy;
  fPosition = position;
...
void Run::EndOfRun()
{
  ...
  for(int i=6;i<30;i++){
  analysisManager->ScaleH1(i,factor);
  }
  for(int i=1;i<6;i++){
  analysisManager->ScaleH2(i,factor);
  }
}
